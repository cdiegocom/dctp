"""
Full experiment on the Adult Census Income dataset.

Demonstrates the Data-Centric Trust Pipeline on a canonical fairness benchmark.
Produces:
  - results/tables/adult_summary.json
  - results/provenance_graphs/adult_full.json
  - results/figures/adult_*.png
"""
...
DATA_PATH = Path("/home/claude/dctp/data/adult.csv")
RESULTS = Path("/home/claude/dctp/results")
SEED = 42

# Plausibility rules for Adult (Protocol P4):
#  - age between 17 and 90 (Adult is working-age population)
#  - hours-per-week between 1 and 99
#  - education-num between 1 and 16
PLAUSIBILITY_RULES = [
...
def main():
    print(f"Loading Adult from {DATA_PATH}")
    df = pd.read_csv(DATA_PATH)
    print(f"Loaded: {df.shape}")

    # Schema: dtype per column
    schema = {
    ...
    synthesis = SynthesisLayer(
        js_divergence_threshold=0.10,  # strict per paper Table 4
        p4_min_pass_rate=0.95,
        random_state=SEED,
    )

    pipeline = TrustPipeline(integrity, fairness, synthesis)
    print("\nRunning pipeline on full Adult dataset...")
    result = pipeline.run(
        df,
        dataset_name="adult_full",
    ...
    summary_path = RESULTS / "tables" / "adult_summary.json"
    summary_path.parent.mkdir(parents=True, exist_ok=True)
    with open(summary_path, "w") as f:
        json.dump(summary, f, indent=2, default=str)
    print(f"\nSummary saved to {summary_path}")

    prov_path = result.provenance.save(RESULTS / "provenance_graphs" / "adult_full.json")
    ...
    # Figure: baseline vs final EO gaps per attribute
    ...
    # Figure: per-group TPR before/after
    ...
    # Figure: P1 JS divergence per batch
    ...
    bars = ax.barh(labels, p1_vals, color=["#27ae60" if v <= 0.10 else "#c0392b"
                                            for v in p1_vals])
    ax.axvline(x=0.10, color="black", linestyle="--", linewidth=1, label="P1 threshold (0.10)")
    ax.set_xlabel("Jensen-Shannon divergence (synthetic vs source)")
